...
export function Navbar() {
  ...
  return <header className="sticky top-0 z-50 border-b border-brand/10 bg-cream/95 backdrop-blur-xl">
    <div className="container-page flex h-20 items-center justify-between gap-3">
      ...
      <div className="flex items-center gap-3"><Link href="/favoritos" className="hidden py-3 text-sm font-bold text-muted sm:block">♡ Favoritos</Link><Link href="/login" className="hidden py-3 text-sm font-bold text-muted lg:block">Iniciar sesión</Link><div className="hidden sm:block"><ButtonLink href="/publicar">Publicá tu servicio</ButtonLink></div><button type="button" aria-expanded={open} aria-controls="mobile-navigation" aria-label={open ? "Cerrar menú" : "Abrir menú"} onClick={() => setOpen(!open)} className="grid h-11 w-11 place-items-center rounded-full border border-brand/20 text-xl text-brand xl:hidden">{open ? "×" : "☰"}</button></div>
    </div>
    {open && <nav id="mobile-navigation" aria-label="Navegación móvil" className="container-page grid max-h-[calc(100dvh-5rem)] gap-1 overflow-y-auto border-t border-brand/10 py-4 xl:hidden">{[...links, ["Favoritos", "/favoritos"], ["Iniciar sesión", "/login"], ["Publicá tu servicio", "/publicar"]].map(([label, href]) => <Link key={href} href={href} onClick={() => setOpen(false)} className="rounded-xl px-3 py-3 text-sm font-bold hover:bg-mint">{label}</Link>)}</nav>}
  </header>;
}
