...
export function FavoritesList({ providers, emptyTitle = "Todavía no guardaste proveedores" }: { providers: Provider[]; emptyTitle?: string }) {
  const [shared, setShared] = useState(false);
  const { favoriteIds } = useFavorites();
  const favorites = providers.filter((provider) => favoriteIds.includes(provider.id));

  if (!favorites.length) return <div className="mx-auto max-w-lg rounded-[2rem] bg-white p-10 text-center soft-shadow md:p-12"><span className="text-5xl text-brand">♡</span><h2 className="display mt-5 text-3xl font-semibold">{emptyTitle}</h2><p className="mt-3 text-muted">Usá el corazón de las cards o perfiles para armar tu selección.</p><ButtonLink href="/servicios" className="mt-7">Explorar servicios</ButtonLink></div>;
