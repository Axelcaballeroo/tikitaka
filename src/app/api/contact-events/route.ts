...
export async function POST(request: Request) {
  ...
  if (
    ...
    !CONTACT_EVENT_SOURCES.includes(source as ContactEventSource) ||
    page.length > 200
  )
    return NextResponse.json({ error: "Datos inválidos." }, { status: 400 });
  try {
    await createContactEvent({
      providerId,
      source: source as ContactEventSource,
      page,
    });
    return new NextResponse(null, { status: 204 });
  } catch (error) {
    console.warn(
      "[Tiki Taka] No se pudo registrar el contacto:",
